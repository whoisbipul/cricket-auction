...
export async function loginUser() {
    ...
    try { const res = await signInWithEmailAndPassword(auth, e, p); const userDoc = await getDoc(doc(db, "users", res.user.uid)); if(userDoc.data().role === 'admin') window.location.href = "admin.html"; else window.location.href = "auction.html"; } catch(err) { alert("Error: " + err.message); }
}
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        currentUser = user; const userDoc = await getDoc(doc(db, "users", user.uid)); currentRole = userDoc.data().role; currentTeamId = userDoc.data().teamId || null;
        const ac = document.getElementById('admin-controls'), tc = document.getElementById('team-controls');
        if (ac && currentRole !== 'admin') ac.classList.add('hidden'); if (tc && currentRole === 'admin') tc.classList.add('hidden');
        syncRules();
    }
});
...
// Auction Engine (Random fix & Timer)
export async function nextPlayer() {
    ...
    const snap = await getDocs(q); if (snap.empty) return alert("Category Done!");
